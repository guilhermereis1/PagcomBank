 require 'rails_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to test the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator. If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails. There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.

RSpec.describe "/balances", type: :request do
  
  # Balance. As you add validations to Balance, be sure to
  # adjust the attributes here as well.
  let(:valid_attributes) {
    skip("Add a hash of attributes valid for your model")
  }

  let(:invalid_attributes) {
    skip("Add a hash of attributes invalid for your model")
  }

  describe "GET /index" do
    it "renders a successful response" do
      Balance.create! valid_attributes
      get balances_url
      expect(response).to be_successful
    end
  end

  describe "GET /show" do
    it "renders a successful response" do
      balance = Balance.create! valid_attributes
      get balance_url(balance)
      expect(response).to be_successful
    end
  end

  describe "GET /new" do
    it "renders a successful response" do
      get new_balance_url
      expect(response).to be_successful
    end
  end

  describe "GET /edit" do
    it "render a successful response" do
      balance = Balance.create! valid_attributes
      get edit_balance_url(balance)
      expect(response).to be_successful
    end
  end

  describe "POST /create" do
    context "with valid parameters" do
      it "creates a new Balance" do
        expect {
          post balances_url, params: { balance: valid_attributes }
        }.to change(Balance, :count).by(1)
      end

      it "redirects to the created balance" do
        post balances_url, params: { balance: valid_attributes }
        expect(response).to redirect_to(balance_url(Balance.last))
      end
    end

    context "with invalid parameters" do
      it "does not create a new Balance" do
        expect {
          post balances_url, params: { balance: invalid_attributes }
        }.to change(Balance, :count).by(0)
      end

      it "renders a successful response (i.e. to display the 'new' template)" do
        post balances_url, params: { balance: invalid_attributes }
        expect(response).to be_successful
      end
    end
  end

  describe "PATCH /update" do
    context "with valid parameters" do
      let(:new_attributes) {
        skip("Add a hash of attributes valid for your model")
      }

      it "updates the requested balance" do
        balance = Balance.create! valid_attributes
        patch balance_url(balance), params: { balance: new_attributes }
        balance.reload
        skip("Add assertions for updated state")
      end

      it "redirects to the balance" do
        balance = Balance.create! valid_attributes
        patch balance_url(balance), params: { balance: new_attributes }
        balance.reload
        expect(response).to redirect_to(balance_url(balance))
      end
    end

    context "with invalid parameters" do
      it "renders a successful response (i.e. to display the 'edit' template)" do
        balance = Balance.create! valid_attributes
        patch balance_url(balance), params: { balance: invalid_attributes }
        expect(response).to be_successful
      end
    end
  end

  describe "DELETE /destroy" do
    it "destroys the requested balance" do
      balance = Balance.create! valid_attributes
      expect {
        delete balance_url(balance)
      }.to change(Balance, :count).by(-1)
    end

    it "redirects to the balances list" do
      balance = Balance.create! valid_attributes
      delete balance_url(balance)
      expect(response).to redirect_to(balances_url)
    end
  end
end
